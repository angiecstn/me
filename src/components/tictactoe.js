...
class TicTacToe extends Component {
	...
	checkGameOver(board, p1turn) {
		var gameOver = false;
		gameOver = this.checkCross(board);
		for (var n = 0; n < 3; n++) {
		...
		return(
			<div>
				...
				<div className="gamebox">
					<div className="row">
						<div onClick={ (e) => { this.onBoxClick(0,0)} } className="box">{ FIRST_ROW[0] }</div>
						<div onClick={ (e) => { this.onBoxClick(1,0)} } className="box">{ FIRST_ROW[1] }</div>  
						<div onClick={ (e) => { this.onBoxClick(2,0)} } className="box">{ FIRST_ROW[2] }</div>
					</div>
					<div className="row">
						<div onClick={ (e) => { this.onBoxClick(0,1)} } className="box">{ SEC_ROW[0]}< /div>
						<div onClick={ (e) => { this.onBoxClick(1,1)} } className="box">{ SEC_ROW[1]}< /div>  
						<div onClick={ (e) => { this.onBoxClick(2,1)} } className="box">{ SEC_ROW[2]}< /div>					
					</div>
					<div className="row">
						<div onClick={ (e) => { this.onBoxClick(0,2)} } className="box">{ THRD_ROW[0]} </div>
						<div onClick={ (e) => { this.onBoxClick(1,2)} } className="box">{ THRD_ROW[1]} </div>  
						<div onClick={ (e) => { this.onBoxClick(2,2)} } className="box">{ THRD_ROW[2]} </div>					
					</div>
				</div>
